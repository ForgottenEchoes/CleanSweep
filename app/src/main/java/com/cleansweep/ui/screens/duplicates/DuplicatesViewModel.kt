...
class DuplicatesViewModel @Inject constructor(
    ...
    override fun onCleared() {
        if (stateHolder.state.value.scanState != BackgroundScanState.Scanning) {
            Log.d("DuplicatesViewModel", "ViewModel cleared. Resetting ScanStateHolder.")
            stateHolder.reset()
        }
        super.onCleared()
    }
    ...
    fun startScan() {
        ...
        _uiState.update {
            it.copy(
                scanState = ScanState.Scanning,
                scanProgress = 0f,
                scanProgressPhase = "Preparing...", // Initial phase
                resultGroups = emptyList(),
                unscannableFiles = emptyList(),
                selectedForDeletion = emptySet(),
                spaceToReclaim = 0L,
                detailedGroup = null,
                showUnscannableSummaryCard = true // Reset for new scan
            )
        }
            ...
            if (success) {
                ...
                val remainingGroups = _uiState.value.resultGroups.mapNotNull { group ->
                    ...
                    if (remainingItems.size > 1) {
                        when (group) {
                            is ExactDuplicateGroup -> group.copy(items = remainingItems)
                            is SimilarMediaGroup -> group.copy(items = remainingItems)
                        }
                    } else {
