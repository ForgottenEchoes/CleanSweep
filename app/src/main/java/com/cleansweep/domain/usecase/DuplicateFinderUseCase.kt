...
class DuplicateFinderUseCase @Inject constructor(
    ...
    suspend fun findDuplicates(
        ...
        // --- Phase 2: Hashing ---
        val cache = fileHashDao.getAllHashes().associateBy { it.filePath }
        val finalHashes = mutableMapOf<String, MutableList<MediaItem>>()
        val hashesToUpsert = mutableListOf<FileHashCache>()
                    ...
                    val hash = if (cached != null && cached.lastModified == mediaItem.dateModified && cached.size == mediaItem.size) {
                        ...
                        try {
                            ...
                            if (newHash.isNotEmpty()) {
                                hashesToUpsert.add(FileHashCache(mediaItem.id, mediaItem.dateModified, mediaItem.size, newHash))
                            }
                            newHash
                        } catch (e: Exception) {
        ...
        if (hashesToUpsert.isNotEmpty()) {
            fileHashDao.upsertHashes(hashesToUpsert)
        }
        val deletedPaths = cache.keys - allScannedPaths
        if (deletedPaths.isNotEmpty()) {
            fileHashDao.deleteHashesByPath(deletedPaths.toList())
        }
        ...
        val resultGroups = finalHashes.values
            ...
            .map { items ->
                ExactDuplicateGroup(
                    hash = finalHashes.entries.first { it.value === items }.key,
